...
int tcp_passive_open(tcpsock_t **sock, int port)
{
    ...
    TCP_ERR_HANDLER(((port < MIN_PORT) || (port > MAX_PORT)), return TCP_ADDRESS_ERROR); /* Checking the validity of port number */
    tcpsock_t *s = tcp_sock_create();                                                    /* Init the socket to default values*/
    TCP_ERR_HANDLER(s == NULL, return TCP_MEMORY_ERROR);
    s->sd = socket(PROTOCOLFAMILY, TYPE, PROTOCOL);                                              /* Create an endpoint socket for communication: Step 1: socket() */
    TCP_DEBUG_PRINTF(s->sd < 0, "Socket() failed with errno = %d [%s]", errno, strerror(errno)); /* Checking the value of socket description */
    TCP_ERR_HANDLER(s->sd < 0, free(s); return TCP_SOCKOP_ERROR);                                /* Free socket */
    ...
    /* Assigning the address specified to the socket referred to by the file description s->sd: Step 2: bind() */
...
{
    struct sockaddr_in addr; /* Storing the client address */
    tcpsock_t *s;
    ...
    s = tcp_sock_create();
    TCP_ERR_HANDLER(s == NULL, return TCP_MEMORY_ERROR);
    s->sd = accept(socket->sd, (struct sockaddr *)&addr, &length); /* Accept a connection on a socket: Step 4: accept() */
...
int tcp_wait_for_connection_remake(int sockfd, tcpsock_t **new_socket)
{
    struct sockaddr_in addr; /* Storing the client address */
    tcpsock_t *s;
    unsigned int length = sizeof(struct sockaddr_in);
    ...
    s = tcp_sock_create();
    TCP_ERR_HANDLER(s == NULL, return TCP_MEMORY_ERROR);
    s->sd = accept(sockfd, (struct sockaddr *)&addr, &length); /* Accept a connection on a socket: Step 4: accept() */
    TCP_DEBUG_PRINTF(s->sd == -1, "Accept() failed with errno = %d [%s]", errno, strerror(errno));
    TCP_ERR_HANDLER(s->sd == -1, free(s); return TCP_SOCKOP_ERROR);
    ...
    *new_socket = s;
    ...
    /* Print Client IP */
    getpeername(s->sd, (struct sockaddr *)&addr, sizeof(addr));
